...
public class BlockDoublePlant extends BlockBush implements IGrowable {
    ...
    public boolean isReplaceable(final World worldIn, final BlockPos pos) {
        ...
        if (iblockstate.getBlock() != this) {
            ...
            final EnumPlantType blockdoubleplant$enumplanttype = this.getActualState(iblockstate, worldIn, pos).getValue(VARIANT);
    ...
    protected void checkAndDropBlock(final World worldIn, final BlockPos pos, final IBlockState state) {
        if (!this.canBlockStay(worldIn, pos, state)) {
            final boolean flag = state.getValue(HALF) == EnumBlockHalf.UPPER;
    ...
    public Item getItemDropped(final IBlockState state, final Random rand, final int fortune) {
        if (state.getValue(HALF) == EnumBlockHalf.UPPER) {
            ...
            final EnumPlantType blockdoubleplant$enumplanttype = state.getValue(VARIANT);
        ...
        if (state.getValue(HALF) == EnumBlockHalf.UPPER) {
            if (worldIn.getBlockState(pos.down()).getBlock() == this) {
                if (!player.capabilities.isCreativeMode) {
                    ...
                    final EnumPlantType blockdoubleplant$enumplanttype = iblockstate.getValue(VARIANT);
