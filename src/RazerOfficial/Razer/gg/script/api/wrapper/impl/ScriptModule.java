...
import RazerOfficial.Razer.gg.script.api.RenderAPI;
...
public final class ScriptModule extends ScriptHandlerWrapper<Module> {
    private final Function<Event, Boolean> eventListenerFunction = event -> {
        if (!Razer.INSTANCE.getModuleManager().contains(this.wrapped)) return true;
    ...
    public void unregister() {
        ...
        Razer.INSTANCE.getModuleManager().remove(this.wrapped);
        Razer.INSTANCE.getStandardClickGUI().rebuildModuleCache();
    }
...
//        Razer.INSTANCE.getEventBus().registerCustom(eventListenerFunction); // changing when switching event system
    }
